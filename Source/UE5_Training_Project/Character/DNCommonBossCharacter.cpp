...
void ADNCommonBossCharacter::fire_1(ADNCommonCharacter* target_in)
{
	...
	if (OBJECT_MANAGER->_enemy_missile_array.Num() < 150)				//1000발까지는 그냥 생성, 1001발째부터는 생성안하고 기존의 500발중 언액티브된 미사일 발사
	{
...
void ADNCommonBossCharacter::fire_2(ADNCommonCharacter* target_in)
{
	...
	if (OBJECT_MANAGER->_enemy_missile_array.Num() < 150)				//500발까지는 그냥 생성, 1001발째부터는 생성안하고 기존의 500발중 언액티브된 미사일 발사
	{
		for (int i = 0; i < 30; ++i)
		{
...
void ADNCommonBossCharacter::target_circle_pattern_spawn(ADNCommonCharacter* target_in)
{
	// 생성 위치
