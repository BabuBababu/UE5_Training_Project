...
#include "UE5_Training_Project/AI/DNRaptureCubeAttackTask.h"
...
UDNRaptureCubeAttackTask::UDNRaptureCubeAttackTask(FObjectInitializer const& object_initializer)
{

	NodeName = TEXT("RaptureCubeAttackTask");
	_limit_time = 2.f;
	_current_time = 0.f;
}


EBTNodeResult::Type UDNRaptureCubeAttackTask::ExecuteTask(UBehaviorTreeComponent& owner_comp_in, uint8* NodeMemory_in)
{
...
void UDNRaptureCubeAttackTask::TickTask(UBehaviorTreeComponent& owner_comp_in, uint8* NodeMemory_in, float DeltaSeconds)
{
	...
	// 캐릭터
	ADNRaptureCubeCharacter* self_actor = Cast<ADNRaptureCubeCharacter>(self_pawn);
	...
	if (_limit_time < _current_time)
	{
		...
		//FinishLatentTask(owner_comp_in, EBTNodeResult::InProgress);
		_current_time = 0.f;
	}
	else
